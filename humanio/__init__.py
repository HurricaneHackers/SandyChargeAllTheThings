...
class App(object):
    ...
    def __init__(self, developer_id, secret_key, app_id=None,
        ...
        :param app_id: You can optionally supply a string to identify this app by.  The string is namespaced to your developer account, so it will not conflict with other developer's.  If you do not provide an app_id, a random app_id will be generated. In general you should specify both app_id and task_id (in create_task) or neither.
...
"""
    ...
    def create_task(self, task_id=None, description=None, hidden=False, thumbnail=None,
        ...
        :param task_id: The ID of the task to create. If not specified, a random unique string is generated. For demos and tests this parameter can be left out, but for real apps it should be specified so that the data on the server can persist when re-connecting the app. In general you should specify both app_id (when creating a new app) and task_id (here) or neither.
    ...
    def post_photo_to_tumblr(self, blog_name, photo_url, caption_html=None, tags=None):
        """
        Post the specified photo (by URL) to a Tumblr blog. As a developer you must have already connected your Human.io developer account to your Tumblr account. You can do this on the Human.io site in your developer page.

        :param blog_name: The full blog name, such as "example.tumblr.com".
        :type blog_name: string
        :param photo_url: The full URL of the photo to post. You can pass in a URL returned from a Human.io photo widget.
        :type photo_url: string
        :param caption_html: Optional caption to show with the photo. May contain HTML. Be sure to escape any HTML entities such as ampersands.
        :type caption_html: string
        ...
        params = {
            "blog_name": blog_name,
            "photo_url": photo_url,
            "caption_html": caption_html or "",
            "tags": tags or "",
        }

        self._call_api("tumblr/photo", params)

    def add_row_to_google_spreadsheet(self, worksheet_id, items):
        """
        Add a row of data to an existing Google Spreadsheet. You must first connect your Human.io developer account to your Google account (http://human.io/developer). You must then create the spreadsheet and name the columns in the first row. See http://human.io/developer/google_spr.

        :param worksheet_id: The ID of the worksheet to add the row to. This looks like a full URL. This page can help you determine this value: http://human.io/developer/google_spr
        :type worksheet_id: string
        :param items: A dictionary of key/value pairs to add. The key is searched in the first row of each column. Keys that don't correspond to column names are ignored. Keys (and column names) are case-sensitive, must contain only letters, numbers, periods, and hyphens, and must start with a letter.
        :type items: dict mapping column name to cell value
        """
        ...
        self._call_api("google_spr/add_row", params)

    def reverse_geo(self, latitude, longitude):
        """
        Given a latitude and longitude, returns a string that describes the location. This is usually a city and state, but in general the format and content can't be depended on except that it will be meaningful to a human reading it. May return an empty string if the location cannot be determined or if the Human.io server is over quota retrieving this information from Google's API.

        :param latitude: The latitude in degrees as a floating point number.
        :type latitude: float
        :param longitude: The longitude in degrees as a floating point number.
        ...
        http_response = self._call_api("geo/reverse", params)
    ...
    def start_loop(self):
        ...
        while self.running:
            ...
            self._handle_raw_response(results["text"], results["status_code"])
    ...
    def send_notification(self, task_id, human_id, alert=None, badge=None,
                          should_launch_task=False, parameters=None):
        """
        (THIS FEATURE IS STILL UNDER DEVELOPMENT AND MAY CHANGE. DO NOT USE.) Sends a push notification to a user's device. Notifications are limited to 256 bytes, including the alert message and all other parameters. Notifications above this limit will be rejected. Delivery of notifications is "best effort" and not guaranteed. It is possible that the user will have multiple devices. Returns False if the notification could not be sent.

        :param task_id: Task associated with the notification.
        :type task_id: string
        :param human_id: Human ID to send the notification to.
        :type human_id: string
        :param alert: Optional message to display in the notification.
        :type alert: string
        :param badge: Optional number to display in a badge for the task icon.
        :type badge: integer
        :param should_launch_task: If True, the task is launched if the user taps on the notification.
        :type should_launch_task: boolean
        :param parameters: Optional dictionary of custom data to pass along to the launched task. Accessible within the task as session.notification_parameters.
        ...
        if should_launch_task:
            params["should_launch_task"] = 1

        result = self._call_api("notification/send", params)
        status_code = result["status_code"]
        if status_code != 200:
            print "Could not send notification. (Status code %s)" % status_code
            return False
        else:
            text = json.loads(result["text"])
            if text["status"] != "ok":
                print "Could not send notification. (%s)" % text["reason"]
                return False

        return True

    def _is_gateway_timeout(self, code):
        # Nginx timed out. This isn't a problem, just try again.
        return code == 599 or code == 504

    def _is_server_restart(self, code):
        # The Human.io server restarted. Wait a bit then retry.
        return code == -3 or code == 502

    def _handle_raw_response(self, text, code):
        if code == 200:
            try:
                self.handle_response(text)
            except Exception as e:
                print "Got exception (%s) handling next event" % e
                time.sleep(1)
        elif self._is_gateway_timeout(code):
            # No problem.
            pass
        elif self._is_server_restart(code):
            # Pause a bit then try again.
            time.sleep(1)
        else:
            # Unusual situation. Sleep and try again.
            print "Got code %d (%s) getting next event" % (code, text)
            time.sleep(1)
...
class AppTornado(App):
    ...
    def _fetch_response_callback(self, http_response):
        self._handle_raw_response(http_response.body, http_response.code)
...
class Session(object):
    ...
    * camera: True if the client app has a camera.
    ...
    def reset_badge(self):
        """
        (THIS FEATURE IS STILL UNDER DEVELOPMENT AND MAY CHANGE. DO NOT USE.) Remove a task's badge from the task list.
        """
    ...
    def set_badge(self, badge):
        """
        (THIS FEATURE IS STILL UNDER DEVELOPMENT AND MAY CHANGE. DO NOT USE.) Set the badge number for the task in the task list.

        :param badge: The number to display on the badge. If 0, no badge is displayed.
        :type badge: integer
        """
        ...
        self.task.app._call_api("session/%s/set_task_badge" % self.session_id, params)
